...
import api from '../api';
import type { Person, Seller } from '../types';
import AddSeller from '../components/AddSeller';
import SellerCard from '../components/SellerCard';

export default function Seller() {
    const [sellers, setSeller] = useState<Seller[]>([]);
    ...
    const cargarTablero = async () => {
        try {
            const [resSeller, resPersons] = await Promise.all([
    ...
    const handleLeads = async (e: React.FormEvent) => {
        e.preventDefault();
        navigate('/leads');
    };
        ...
        <div className="min-h-screen w-full bg-[#020205] bg-[radial-gradient(circle_at_center,_#1a1b3a_0%,_#020205_100%)] text-white font-sans p-8">
            <header className="flex flex-col md:flex-row justify-between items-center mb-12 pb-6 border-b border-white/10 gap-6">
                ...
                <nav className="flex items-center gap-4">
                    <button 
                        onClick={handleLeads}
                        className="px-6 py-2 rounded-full bg-white/5 border border-white/10 text-gray-300 font-bold hover:bg-white/10 transition-all cursor-pointer">
                        Clientes
                    </button>
                    <button 
                    ...
                    <button 
                        ...
                        className="px-6 py-2 rounded-full bg-red-500/10 border border-red-500/30 text-red-400 font-bold hover:bg-red-500/20 transition-all cursor-pointer">
                        Cerrar sesión
                    </button>
                </nav>
            </header>
            <div className="flex flex-col lg:flex-row gap-10 items-start">
                <aside className="w-full lg:w-[450px] lg:sticky lg:top-8">
                    <div className="relative group">
                        ...
                        <AddSeller onSuccess={cargarTablero} />
                    </div>
                </aside>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3 gap-6">
                            {sellers.map((seller) => (
                                <SellerCard
                                    key={seller.id}
                                    seller={seller}
                                    person={getPersonData(seller.person)} 
